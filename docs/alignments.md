...
If the user wishes to use a single process, as in the previous version of the workflow, they can do so by providing `--full_alignment`. This will perform a single genome-to-genome process. You might want to increase the number of cores provided to minimap2 with `--minimap2_threads`.
